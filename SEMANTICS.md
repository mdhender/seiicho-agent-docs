...
A cluster is a cube with edge length of 31 units.
...
4. The cluster is initialized with a slice of 100 lattice points in [1..31]^3 such that no two are within Chebyshev distance <= 1 (i.e., they are not the same cell, face-adjacent, edge-adjacent, or 3D-diagonal neighbors).

5. The 100 lattice points are generated by building a list of all possible points with integer values for (X, Y, Z), shuffling the list uniformly, drawing candidates from the list until the list is exhausted or we have collected 100 points.

6. A candidate lattice point is **rejected** if any of the following hold:

    * distance from candidate to any previously accepted lattice point is within Chebyshev distance <= 1

7. Once accepted, the lattice point is added to the list of accepted lattice points.

8. It is a hard error if the list is exhausted before we have accepted 100 lattice points.

### Cluster Generation Signature

```pseudocode
// LatticePoint is an integer 3D coordinate in the inclusive range [1..31] for each axis.
